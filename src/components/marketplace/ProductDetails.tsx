...
const ProductDetails: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center p-6">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-4xl flex flex-col md:flex-row gap-4">
        <img src={product.image_url} alt={product.name} className="w-full md:w-1/2 rounded-lg object-cover h-96" />
        <div className="flex flex-col gap-2 md:w-1/2">
          <h1 className="text-2xl font-semibold">{product.name}</h1>
          <p className="text-green-600 font-bold">Price: Rs. {product.price}</p>
          <p><strong>Available:</strong> {product.quantity} kg</p>
          <p><strong>Listed on:</strong> {product.created_at}</p>
          <h2 className="text-lg font-semibold mt-4">Seller Info</h2>
